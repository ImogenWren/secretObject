/* Class for managing storing and retreving settings & memStruc_t data from persistant memory
...
#define SECRETOBJECT_VERSION "V2.0.0"  // backwards compatable with 1.0.0? No Guarentee
#define EEPROM_START_ADDRESS 0x10      // define start point for any EEPROM persistant memory use
#define FLASH_WRITES_MAX 25            // define number of writes untill firmware must be re-programmed
#define SECRET_LEN_MAX 9

#define PROGRAM_SIGNATURE 0x00AAF  // // change this when re-writing firmware to enable overwrite of memStruc_t data
...
public:
  ...
  // Public memStruc_t struct -> modify for specific use case (needed to pass cal data + flag out of library)
  struct calStruc_t {
    bool calValid;
    int16_t calData;
  };
  ...
  // jsonMessenger has to change to accomidate materials char string with auth code
    ...
    // Private cal struc -> stores the cal data in persistant memory, keep definition public but the instance private to prevent library users accidently passing this structure out of the library and exposing the secret
  struct memStruc_t {
    ...
    bool calValid;  // Set this true when memStruc_t data is first written -> want to change this to calValid, but preserving backwards compatability
    uint8_t settingsValid;
    char secret[SECRET_LEN_MAX + 1];  // Secret string for authorising memStruc_t updates // now -> 8 char password (too long ->)(typically a uuid of 36 chars in form 8-4-4-4-12)
    ...
    int16_t angleMax;
    int16_t loadMax;
    int16_t calData;  // Scale factors / memStruc_t / offset data
  };
  ...
  bool cal_is_valid();
  bool settings_valid();
  bool check_secret(const char *secret);     // Lets us borrow secret functions for use outside this library
  secretObject::calStruc_t get_cal();
  secretObject::settingsStruc_t get_settings();
...
private:
  ...
  // Create a global "memStruc_t" variable and call it cal make this private to prevent it being passed directly out of library
  memStruc_t cal;

  // Reserve a portion of flash memory to store a "memStruc_t" and
  // call it "cal_store".
  // dont think this works
  //FlashStorage(cal_store, memStruc_t);  // SAMD21 method only

  // Private Functions
// helpers
  void memory_write(secretObject::memStruc_t new_data);  // System agnostic helper function for memory writes (only need to update this function to add additional methods for different systems)
  secretObject::memStruc_t memory_get();


  bool cal_pre_check(secretObject::memStruc_t calStruc_t, const char *secret);
